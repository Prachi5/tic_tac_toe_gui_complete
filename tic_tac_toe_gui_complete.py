...
def evNew():
    ...
    game2cells(tic_tac_toe_logic.newGame())

def evResume():
     status['text'] = "Playing game"
     game = tic_tac_toe_logic.restoreGame()
     game2cells(game)
def evSave():
    game = cells2game()
    tic_tac_toe_logic.saveGame(game)
...
def evHelp():
    mb.showinfo("Help",'''
    File->New: starts a new game of tic-tac-tac
...
def evAbout():
    mb.showinfo("About","Tic-tac-toe game GUI demo by Prachi Raj")
def evClick(row,col):
